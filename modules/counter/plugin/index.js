...
function resolveProps(props = {}) {
    const resolvedProps = {
        ...
        widgetBundleIdSuffix: props.widgetBundleIdSuffix ?? DEFAULT_PROPS.widgetBundleIdSuffix,
    };
...
function validateProps(props) {
    ...
    if (!appGroupPattern.test(props.appGroup)) {
        throw new Error(`[counter plugin] Invalid appGroup "${props.appGroup}". Expected format like "group.com.example.app".`);
    }
    const xcodeNamePattern = /^[A-Za-z][A-Za-z0-9_]*$/;
    if (!xcodeNamePattern.test(props.widgetName)) {
        throw new Error(`[counter plugin] Invalid widgetName "${props.widgetName}". Use letters, numbers, and underscores, starting with a letter.`);
    }
    const suffixPattern = /^[A-Za-z0-9._-]+$/;
    if (!suffixPattern.test(props.widgetBundleIdSuffix)) {
        throw new Error(`[counter plugin] Invalid widgetBundleIdSuffix "${props.widgetBundleIdSuffix}". Use letters, numbers, dot, underscore, or hyphen.`);
    }
    const normalizedSuffix = normalizeBundleIdSuffix(props.widgetBundleIdSuffix);
    if (!normalizedSuffix) {
        throw new Error(`[counter plugin] widgetBundleIdSuffix "${props.widgetBundleIdSuffix}" normalizes to an empty value.`);
    }
}
function getWidgetPaths(projectRoot, iosRoot, resolvedProps) {
    const sourceRoot = node_path_1.default.join(projectRoot, "modules", "counter", "ios-widget");
    const sourceWidgetRoot = node_path_1.default.join(sourceRoot, "CounterWidget");
    const sourceTemplatesRoot = node_path_1.default.join(sourceRoot, "templates");
    const destinationWidgetRoot = node_path_1.default.join(iosRoot, resolvedProps.widgetName);
    const destinationEntitlements = node_path_1.default.join(iosRoot, `${resolvedProps.widgetBundleIdSuffix}.entitlements`);
    const infoPlistTemplatePath = node_path_1.default.join(sourceTemplatesRoot, WIDGET_INFO_PLIST_TEMPLATE);
    const infoPlistDestinationPath = node_path_1.default.join(destinationWidgetRoot, "Info.plist");
    return {
        sourceWidgetRoot,
...
function ensureWidgetSourcesMembership(project, widgetFiles, widgetName, widgetTargetUuid) {
    ...
    const mainSourcesUuid = mainTargetUuid
        ...
        : null;
    const widgetSourcesUuid = getBuildPhaseUuidByTargetUuid(project, widgetTargetUuid, "PBXSourcesBuildPhase");
    ...
    for (const [fileName, fileRefUuid] of fileRefByName.entries()) {
        ...
        const alreadyInWidgetSources = widgetSourcePhase.files.some((entry) => entry.value === ensuredBuildFileUuid);
        ...
        if (mainSourcesUuid && sourcesPhases[mainSourcesUuid]?.files) {
            sourcesPhases[mainSourcesUuid].files = sourcesPhases[mainSourcesUuid].files.filter((entry) => entry.value !== ensuredBuildFileUuid);
        }
    }
}
...
function getHostVersionSettings(project) {
    const mainTarget = project.getFirstTarget();
    const mainTargetUuid = mainTarget?.uuid;
    if (!mainTargetUuid) {
        return {};
...
const withCounterWidget = (config, props = {}) => {
    ...
    config = (0, config_plugins_1.withEntitlementsPlist)(config, (modConfig) => {
    ...
    config = (0, config_plugins_1.withDangerousMod)(config, ["ios", async (modConfig) => {
            const projectRoot = modConfig.modRequest.projectRoot;
            const iosRoot = modConfig.modRequest.platformProjectRoot;
            const { sourceWidgetRoot, destinationWidgetRoot, destinationEntitlements, infoPlistTemplatePath, infoPlistDestinationPath, } = getWidgetPaths(projectRoot, iosRoot, resolvedProps);
            node_fs_1.default.mkdirSync(destinationWidgetRoot, { recursive: true });
            node_fs_1.default.cpSync(sourceWidgetRoot, destinationWidgetRoot, { recursive: true, force: true });
            node_fs_1.default.copyFileSync(infoPlistTemplatePath, infoPlistDestinationPath);
            const sharedConfigPath = node_path_1.default.join(destinationWidgetRoot, "SharedConfig.swift");
            ...
            node_fs_1.default.writeFileSync(sharedConfigPath, sharedConfigContents, "utf8");
            const entitlementsContent = `<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0">\n<dict>\n\t<key>com.apple.security.application-groups</key>\n\t<array>\n\t\t<string>${resolvedProps.appGroup}</string>\n\t</array>\n</dict>\n</plist>\n`;
            node_fs_1.default.writeFileSync(destinationEntitlements, entitlementsContent, "utf8");
            return modConfig;
        }]);
    config = (0, config_plugins_1.withXcodeProject)(config, (modConfig) => {
        ...
        WIDGET_SOURCE_FILES.forEach((fileName) => {
            ...
            const hasReference = Object.values(fileReferences).some((ref) => ref &&
                (ref.path === `"${fileName}"` ||
                    ref.path === fileName ||
        ...
        ensureWidgetSourcesMembership(project, WIDGET_SOURCE_FILES, widgetName, widgetTargetUuid);
...
const plugin = (0, config_plugins_1.createRunOncePlugin)(withCounterWidget, pluginName, pluginVersion);
